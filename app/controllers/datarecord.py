from app.models.user_account import UserAccount, SuperAccount, user
...
class dataBase():
    ...
    def saveInDataBase(cls, us):
        ...
        if ver != False:
            for i in range(len(cls.__dataB)):
                if us.name == cls.__dataB[i]['username']:
    ...
    def verificarLogin(cls, user, password):
        ...
        for loginGet in cls.__dataB:
            print(f"Comparando entrada: {user}/{password} com {loginGet.get('username')}/{loginGet.get('password')}")
            if user == loginGet.get('username') and password == loginGet.get('password'):
                return True
        return False
